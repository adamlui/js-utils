 ...
 */
let log=require("./log"+env.modExt);module.exports={js({srcFiles:e,inputPath:i,inputArg:r,outputArg:s}){let t=require("fs"),o=require("../../minify"+env.modExt).minify,g=require("path"),n=[],l=[];var c;!cli.config.relativeOutput&&t.statSync(i).isDirectory()?(c=o(i,{verbose:!1,mangle:!cli.config.noMangle,comment:cli.config.comment,relativeOutput:!1,recursive:!cli.config.noRecursion,dotFolders:cli.config.includeDotFolders,dotFiles:cli.config.includeDotFiles,rewriteImports:cli.config.rewriteImports,ignores:cli.config.ignores}))&&(c.error?n.push(i):l=[].concat(c)):l=e.map(e=>{var i=o(e,{verbose:!cli.config.quietMode,mangle:!cli.config.noMangle,comment:cli.config});return i.error&&n.push(e),i}).filter(e=>!e.error),cli.config.quietMode||(l?.length?(log.success(cli.msgs.info_minComplete+"!"),log.data(l.length+" "+cli.msgs.info_file+`${1==l.length?"":"s"} ${cli.msgs.info_minified}.`)):console.info(cli.msgs.info_noFilesProcessed+"."),n.length&&(log.error(n.length+" "+cli.msgs.info_file+(1==n.length?"":"s"),cli.msgs.info_failedToMinify+":"),n.forEach(e=>console.info(e)))),l?.length&&(cli.config.copy&&1==l?.length?(log.data(l[0].code),log.ifNotQuiet(`
