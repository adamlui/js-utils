...
» minify-js . output.min.js[0m`),u(),process.exit(1)));var $,d=s.endsWith(".js")&&!t.statSync(s).isDirectory()?[s]:n.findJS(s,{recursive:!app.config.noRecursion,verbose:!app.config.quietMode,ignores:(app.config.ignores?.split(",")??[]).map(e=>e.trim())});if(app.config.dryRun)d.length?(console.info(`
...
${f}o ${app.msgs.helpSection_usage}:[0m`,` ${f}» ${l}${app.cmdFormat}[0m`],pathArgs:[`
