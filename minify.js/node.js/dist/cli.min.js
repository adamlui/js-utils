...
» minify-js . output.min.js`+nc),printHelpCmdAndDocURL(),process.exit(1)),p.endsWith(".js")?[r]:minifyJS.findJS(r,{recursive:!config.noRecursion,verbose:!config.quietMode,ignoreFiles:config.ignoreFiles?.split(",")??[]}));if(config.dryRun)0<s.length?(console.info(`
${by}JS files to be minified:`+nc),s.forEach(e=>console.info(e))):console.info(by+`
No JS files will be minified.`+nc);else{const w=[],x=s.map(e=>{var i=minifyJS.minify(e,{verbose:!config.quietMode,mangle:!config.noMangle,comment:config.comment?.replace(/\\n/g,"\n")});return i.error&&w.push(e),i}).filter(e=>!e.error);x?.forEach(({code:e,srcPath:i})=>{var n=path.join(path.dirname(i),(/so?u?rce?$/.test(path.dirname(i))?"../":"")+(q.endsWith(".js")?path.dirname(q):q||"min")),i=(q.endsWith(".js")&&p.endsWith(".js")?path.basename(q).replace(/(\.min)?\.js$/,""):path.basename(i,".js"))+".min.js",i=path.join(n,i);fs.existsSync(n)||fs.mkdirSync(n,{recursive:!0}),fs.writeFileSync(i,e,"utf8")}),0<x?.length?(printIfNotQuiet(`
${bg}Minification complete!`+nc),printIfNotQuiet(`${bw+x.length} file${1<x.length?"s":""} minified.`+nc)):printIfNotQuiet(by+"No unminified JavaScript files processed."+nc),0<w.length&&(printIfNotQuiet(`
`+br+w.length+" file"+(1<w.length?"s":"")+" failed to minify:"+nc),w.forEach(e=>printIfNotQuiet(e)))}}function printHelpSections(e=["header","usage","pathArgs","flags","paramOptions","infoCmds"]){var i=`[106m[30m ${pkgName.replace(/^@[^/]+\//,"")} ${nc} `;const n={header:[`
...
${bw}o Parameter options:`+nc,'--ignore-files="file1.js,file2.js"   Files to exclude from minification.','--comment="comment"                  Prepend comment to minified code.'],infoCmds:[`
