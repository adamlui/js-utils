...
» minify-js . output.min.js[0m`),j(),process.exit(1)));var y,g=i.endsWith(".js")&&!l.statSync(i).isDirectory()?[i]:n.findJS(i,{recursive:!$.noRecursion,verbose:!$.quietMode,ignores:($.ignores?.split(",")??[]).map(e=>e.trim())});if($.dryRun)g.length?(console.info(`
...
${f+(h.info_minComplete||"Minification complete")}![0m`),b(d+e.length+" "+(h.info_file||"file")+`${1<e.length?"s":""} ${h.info_minified||"minified"}.${r}`)):b(`
${p+(h.info_noFilesProcessed||"No unminified JavaScript files processed")}.[0m`),o.length&&(b(`
...
${d}${docURL}[0m`)}function b(e){$.quietMode||console.info(e)}})();