...
minify() » ${e} does not exist.`);else{const{mangle:i,comment:n,...o}=t;(s.endsWith(".js")?[e]:minifyJS.findJS(e,{...o,verbose:!1})).map(e=>minifyJS.minify(e,t)).filter(e=>!e.error)?.forEach(({code:e,srcPath:i})=>{var n=path.join(path.dirname(i),(/so?u?rce?$/.test(path.dirname(i))?"../":"")+(r.endsWith(".js")?path.dirname(r):r||"min")),i=(r.endsWith(".js")&&s.endsWith(".js")?path.basename(r).replace(/(\.min)?\.js$/,""):path.basename(i,".js"))+".min.js",i=path.join(n,i);fs.existsSync(n)||fs.mkdirSync(n,{recursive:!0}),fs.writeFileSync(i,e,"utf8")})}}module.exports=minify;