...
${p.info_copying||"Copying to clipboard"}...`,h.quietMode||console.info(u),u=(u=Array.isArray(l)?l.join("\n"):l).replace(/"/g,'""'),d={darwin:{binPath:"/usr/bin/pbcopy",args:[]},linux:{binPath:"/usr/bin/xclip",args:["-selection","clipboard"]},win32:{binPath:o.join(process.env.SYSTEMROOT,"System32","WindowsPowerShell","v1.0","powershell.exe"),args:["-Command","Set-Clipboard -Value $input"]}},s(...Object.values(d[process.platform]),{input:u})}function $(o){return new Promise((r,e)=>{var s=o.match(/^([^:]+):\/\//)[1];/^https?$/.test(s)||e(new Error(`${p.error_invalidURL||"Invalid URL"}.`)),require(s).get(o,e=>{let s="";e.on("data",e=>s+=e),e.on("end",()=>r({json:()=>JSON.parse(s)}))}).on("error",e)})}function w(e=["header","usage","paramOptions","flags","infoCmds"]){var s=`[106m[30m ${pkgName} [0m `;let r={header:[`
├ `+s+(p.appCopyright||copyright),s+(p.prefix_source||"Source")+": "+srcURL],usage:[`
${i}o ${p.helpSection_usage||"Usage"}:[0m`,` ${i}» [1;92m${cmdFormat}[0m`],paramOptions:[`
...
${p.info_moreHelp||"For more help"}, ${p.info_visit||"visit"}: ${i}${docURL}[0m`)}function _(){console.info(`
