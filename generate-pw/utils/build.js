...
if (!cli.config.jsOnly) {
    const dataOutDir = 'dist/data',
          filenames = { pkgData: 'package-data.json', msgs: 'messages.json', config: 'generate-pw.config.mjs' }
...
if (!cli.config.dataOnly) {
    ...
    execSync(`npx minify-js src dist --ignores=cli --comment="${cli.headerComment}"`, execOptions)
