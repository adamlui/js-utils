...
参数选项：
 --ignores="dir/,file1.scss,file2.sass"   要从编译中排除的文件/目录。
...
您可以通过在项目根目录放置 `scss-to-css.config.mjs` 或 `scss-to-css.config.js` 文件来自定义 **scss-to-css** 的配置。

默认配置示例：

```js
export default {
    dryRun: false,            // 不实际压缩文件，仅显示哪些文件将被处理
    includeDotFolders: false, // 在文件搜索中包含隐藏文件夹（以点开头的文件夹）
    noSourceMaps: false,      // 阻止生成源映射文件
    noMinify: false,          // 禁用输出 CSS 的压缩
    noRecursion: false,       // 禁用递归文件搜索
    relativeOutput: false,    // 输出文件相对于每个源文件，而不是相对于输入根目录
    copy: false,              // 如果只处理单个文件，则将编译后的 CSS 复制到剪贴板，而不是写入文件
...
💡 运行 `scss-to-css init` 即可在项目根目录生成一个 `scss-to-css.config.mjs` 模板文件。
...
<a href="https://github.com/adamlui/scss-to-css/issues">报告错误</a> /
